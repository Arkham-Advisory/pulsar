      ...
      <div className="flex items-center bg-white dark:bg-slate-900">
        {dragHandleListeners && (
          <button
            ...
            {...dragHandleListeners}
            {...dragHandleAttributes}
            className="pl-3 pr-1 py-1.5 text-slate-300 dark:text-slate-600 hover:text-slate-500 dark:hover:text-slate-400 cursor-grab active:cursor-grabbing shrink-0 touch-none focus:outline-none"
          >
        ...
        <button
          ...
          className={cn(
            'flex-1 flex items-center gap-2 py-1.5 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors text-left',
            dragHandleListeners ? 'pl-2 pr-3' : 'px-3',
          )}
        >
          {open
                    ...
                    {visible.map((pr) => (
                      ...
                      >
                        <PRRow
                          ...
                          isPinned={pinnedPRs?.includes(`${pr.repo}#${pr.number}`)}
                          onTogglePin={onTogglePin ? () => onTogglePin(pr) : undefined}
                        />
                      </motion.div>
                    ))}
                  </AnimatePresence>
